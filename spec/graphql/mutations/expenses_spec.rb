...
RSpec.describe Mutations::Expenses do
  describe described_class::Destroy do
    ...
    context 'when query discards the expense' do
    ...
    context 'when undiscard flag is true' do
      let!(:expense) { create :expense, :discarded }
      let(:user) { expense.day.period.user }
      let(:variables) { { id: expense.id, undiscard: true } }
      let(:query_string) do
        <<-GRAPHQL
        mutation($id: ID!, $undiscard: Boolean) {
          destroyExpense(input: { id: $id, undiscard: $undiscard }) {
            id
          }
        }
        GRAPHQL
