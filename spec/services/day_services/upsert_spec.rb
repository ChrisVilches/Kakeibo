  ...
  describe '#execute' do
    context 'when day does not exist' do
      it 'creates a new one' do
        expect do
          service.execute(period_id: period.id, day_date: day_date, memo: 'good morning')
    ...
    context 'when day exists' do
      before { period.days << build(:day, day_date: day_date, memo: 'hello') }

      it { expect(period.days.first.memo).to eq 'hello' }

      it 'does not create a new one' do
        expect do
          service.execute(period_id: period.id, day_date: day_date, memo: 'bye')
